...
def build_video_spec(
    ...
    resolved_mux_audio_path = _require_readable_file(mux_audio_path, "Mux audio")
    ...
    resolved_output_video_path = (
        ...
        else (run_dir / f"{resolved_mux_audio_path.stem}.mp4").resolve()
    )

    return VideoSpec(
        transcription_path=resolved_transcription_path,
        mux_audio_path=resolved_mux_audio_path,
